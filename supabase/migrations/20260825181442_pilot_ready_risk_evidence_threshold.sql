...
stu as (
  ...
  from public.v2_group_students gs,me where gs.teacher_id=me.uid group by gs.group_id
),
att as (
  select a.group_id,
  ...
  from public.v2_attendance_sessions a left join public.v2_attendance_records r on r.attendance_session_id=a.id,me
  where a.teacher_id=me.uid group by a.group_id
),
today_att as (
...
student_att as (
  ...
  left join public.v2_attendance_records r on r.attendance_session_id=a.id and r.student_id=gs.id,me
...
manual_student as (
  ...
  from public.v2_grade_items i join public.v2_grade_categories c on c.id=i.category_id and c.kind='manual'
  join public.v2_grade_scores s on s.item_id=i.id and s.score is not null,me
...
student_signal as (
  select gs.group_id,gs.id student_id,gs.full_name,gs.enrollment,sa.attendance_rate,coalesce(sa.attendance_count,0) attendance_count,
         os.omr_avg,ms.manual_avg,
  ...
  from public.v2_group_students gs join alerts al on al.group_id=gs.group_id
  ...
  left join manual_student ms on ms.student_id=gs.id and ms.group_id=gs.group_id,me
...
classified as (
  select *,case
    ...
    when (grade_signal is not null and grade_signal<min_grade+1) or (attendance_count>=1 and attendance_rate<min_attendance+5) then 'watch'
    else 'ok' end risk_status
  from student_signal
),
risk as (
  select group_id,count(*) filter(where risk_status='risk')::int risk_count,count(*) filter(where risk_status='watch')::int watch_count
...
groups_json as (
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',g.id,'name',coalesce(g.name,g.group_name),'subject',g.subject,'term',coalesce(g.term,g.school_cycle),'university',g.university,'program',g.program,
    'students',coalesce(stu.students,0),'attendance_rate',att.attendance_rate,'grade_avg',grade_group.grade_avg,'risk_count',coalesce(risk.risk_count,0),'watch_count',coalesce(risk.watch_count,0),
  ...
  from g left join stu on stu.group_id=g.id left join att on att.group_id=g.id left join risk on risk.group_id=g.id
  left join grade_group on grade_group.group_id=g.id left join today_att on today_att.group_id=g.id left join activity on activity.group_id=g.id
),
priority as (
  select coalesce(jsonb_agg(jsonb_build_object('group_id',x.group_id,'student_id',x.student_id,'full_name',x.full_name,'enrollment',x.enrollment,'attendance_rate',x.attendance_rate,'grade',round(x.grade_signal,1),'status',x.risk_status)
    order by case x.risk_status when 'risk' then 0 else 1 end,coalesce(x.attendance_rate,101),coalesce(x.grade_signal,11)),'[]'::jsonb) j
  from(select * from classified where risk_status in('risk','watch') limit 8)x
)
select jsonb_build_object(
  'date',current_date,'groups',groups_json.j,'groups_count',(select count(*) from g),'pending_attendance',(select n from pending),
  'risk_students',(select risk_n from all_risk),'watch_students',(select watch_n from all_risk),'priority_students',(select j from priority),
