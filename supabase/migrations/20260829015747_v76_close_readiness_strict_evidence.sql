...
begin
  ...
  if attendance_weighted and attendance_expected>attendance_records then issues:=issues||jsonb_build_array(jsonb_build_object('code','attendance_pending','label',format('Faltan %s registros de asistencia del periodo.',attendance_expected-attendance_records))); end if;
  if omr_weighted and omr_expected>exam_results then issues:=issues||jsonb_build_array(jsonb_build_object('code','omr_pending','label',format('Faltan %s resultados OMR esperados del periodo.',omr_expected-exam_results))); end if;
  if students_without_grade>0 then issues:=issues||jsonb_build_array(jsonb_build_object('code','students_without_grade','label',format('%s alumnos todavía no tienen promedio del periodo.',students_without_grade))); end if;
