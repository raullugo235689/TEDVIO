...
const teacher=read('teacher.html'),beta=read('beta.html'),boot=read('teacher-progressive-boot-v68.js'),core=read('teacher-core-v68-6.js'),agenda=read('teacher-agenda-v75.js');
...
const allowedCore=['config.js','runtime-core-v64.js','teacher-core-v68-6.js','teacher-progressive-boot-v68.js','teacher-theme-v68-7.js','teacher-command-center-v70.js','teacher-agenda-v75.js'];
const unexpectedDirect=direct.filter(src=>!allowedCore.some(name=>src.includes(name)));
must(direct.length===allowedCore.length&&allowedCore.every(name=>direct.some(src=>src.includes(name)))&&unexpectedDirect.length===0,`teacher first paint remains the audited split-core foundation plus v70 command center and lightweight v75 agenda (${direct.length} direct scripts)`);
...
must(teacher.includes('teacher-agenda-v75.js?v=75'),'teacher loads the explicitly audited v75 agenda controller');
...
console.log('\nTEDVIO v68.4+ compatibility passes with v70 + v75 agenda.');
