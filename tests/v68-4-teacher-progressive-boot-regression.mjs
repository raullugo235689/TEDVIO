...
const teacher=read('teacher.html'),beta=read('beta.html'),boot=read('teacher-progressive-boot-v68.js'),core=read('teacher-core-v68-6.js'),agenda=read('teacher-agenda-v75.js'),periods=read('teacher-periods-v76.js'),router=read('teacher-router-v76-2.js');
...
const allowedCore=['config.js','runtime-core-v64.js','teacher-core-v68-6.js','teacher-progressive-boot-v68.js','teacher-theme-v68-7.js','teacher-command-center-v70.js','teacher-agenda-v75.js','teacher-periods-v76.js','teacher-router-v76-2.js'];
const unexpectedDirect=direct.filter(src=>!allowedCore.some(name=>src.includes(name)));
must(direct.length===allowedCore.length&&allowedCore.every(name=>direct.some(src=>src.includes(name)))&&unexpectedDirect.length===0,`teacher first paint remains the audited split-core foundation plus lightweight academic context/router (${direct.length} direct scripts)`);
...
must(teacher.includes('teacher-agenda-v75.js?v=75')&&teacher.includes('teacher-periods-v76.js?v=76')&&teacher.includes('teacher-router-v76-2.js?v=762'),'teacher loads explicitly audited agenda + period + persistent router controllers');
...
must(!agenda.includes('setInterval(')&&!agenda.includes('new MutationObserver'),'agenda adds no polling or DOM observer');
must(agenda.includes("from('v2_group_schedule_slots')")&&agenda.includes('STATE.lastLoad<30000'),'agenda adds one cached schedule read only');
must(!periods.includes('setInterval(')&&!periods.includes('new MutationObserver'),'period controller adds no polling or DOM observer');
must(periods.includes("from('v2_academic_periods')")&&periods.includes('STATE.lastLoad<30000'),'v76 adds one cached academic-period read');
must(!router.includes('setInterval(')&&!router.includes('new MutationObserver'),'persistent router adds no polling or DOM observer');
must(!/createClient|\.from\(|\.rpc\(/.test(router),'persistent router adds no backend work');
...
console.log('\nTEDVIO v68.4+ compatibility passes with v70 + v75/v76 academic context + v76.2 Quality Core.');
