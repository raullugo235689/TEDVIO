...
const teacher=read('teacher.html'),js=read('teacher-theme-v68-7.js'),css=read('teacher-theme-v68-7.css'),agendaCss=read('teacher-agenda-v75.css'),periodCss=read('teacher-periods-v76.css'),router=read('teacher-router-v76-2.js'),vercel=read('vercel.json');
...
must(scripts.length===allowedScripts.length&&allowedScripts.every(name=>scripts.some(src=>src.includes(name)))&&unexpectedScripts.length===0&&scripts.filter(x=>x.includes('teacher-theme-v68-7.js')).length===1,'theme remains isolated inside the audited v76.2 teacher shell');
...
must(teacher.includes('id="tv762RouterStyle"'),'router presentation is embedded without a second theme stylesheet');
...
console.log('\nTEDVIO v68.7 Dual Theme Premium regression audit passed under v76.2.');
