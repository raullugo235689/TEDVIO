...
const executive=read('beta-executive-v56.css');
...
for(const html of [teacher,beta]){
  must(html.includes('v=56'),'executive shell is cache-busted to v56');
  must(html.includes('tedvio-premium-v54.css?v=56'),'premium design system remains active in v56');
  must(html.includes('beta-pilot-ready-v1.css?v=56'),'Pilot Ready visual layer remains loaded');
  must(html.includes('beta-executive-v56.css?v=56'),'v56 executive visual layer is loaded last');
  must(html.includes('beta-pilot-ready-v1.js?v=56'),'v56 executive runtime is loaded');
...
must(pilot.includes("v2_teacher_today_dashboard"),'Executive dashboard uses server aggregate RPC');
must(pilot.includes("tedvio_client_events"),'Pilot telemetry remains active');
must(pilot.includes('network_offline')&&pilot.includes('unhandled_rejection'),'Pilot runtime monitors connectivity and errors');
must(pilot.includes('attendance_records_count')&&pilot.includes('Sin registros'),'Executive dashboard distinguishes no records from zero attendance');
must(pilot.includes('tvPilotAddStudents')&&pilot.includes('Agregar alumnos'),'Empty groups expose a direct roster setup action');
must(pilot.includes('Siguiente acción')&&pilot.includes('Centro de preparación'),'Executive dashboard exposes contextual next action and onboarding state');
...
must(executive.includes('.tv56-hero')&&executive.includes('.tv56-next')&&executive.includes('.tv56-group-line')&&executive.includes('.tv56-prep'),'v56 executive CSS covers compact hero, next action, groups and preparation center');
...
must(sw.includes("tedvio-pilot-v56-20260825"),'service worker uses the v56 pilot cache namespace');
...
must(String(version.version).endsWith('.56'),'Executive Pilot Ready version is v56');

if(failed){console.error(`\n${failed} audit check(s) failed.`);process.exit(1)}
console.log('\nTEDVIO v56 Executive Pilot Ready static audit passed.');