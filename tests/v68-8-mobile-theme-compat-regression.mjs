...
const teacher=read('teacher.html'),css=read('teacher-mobile-compat-v68-8.css'),agendaCss=read('teacher-agenda-v75.css'),periodCss=read('teacher-periods-v76.css'),theme=read('teacher-theme-v68-7.js'),sw=read('sw.js');
...
must(scripts.length===allowedScripts.length&&allowedScripts.every(name=>scripts.some(src=>src.includes(name)))&&unexpectedScripts.length===0,'v68.8 remains CSS-only inside the explicitly audited v76 teacher shell');
must(styles.length===12&&styles.filter(x=>x.includes('teacher-mobile-compat-v68-8.css')).length===1&&styles.filter(x=>x.includes('teacher-command-center-v70.css')).length===1&&styles.filter(x=>x.includes('teacher-agenda-v75.css')).length===1&&styles.filter(x=>x.includes('teacher-periods-v76.css')).length===1,'v76 adds exactly one responsive period stylesheet to the existing shell');
...
console.log('\nTEDVIO v68.8 Mobile UI & Theme Compatibility regression audit passed under v76.');
