...
const current=fs.existsSync('attendance-institutional-report-v68-11.js')?'attendance-institutional-report-v68-11.js':'attendance-institutional-report-v68-10.js';
const report=read(current),theme=read('teacher-theme-v68-7.js'),teacher=read('teacher.html');
let failed=0;const must=(ok,msg)=>{if(ok)console.log('OK  ',msg);else{console.error('FAIL',msg);failed++}};
must(report.includes("const VERSION='2026.08.28.68.10'")||report.includes("const VERSION='2026.08.28.68.11'"),'institutional attendance report identifies v68.10+');
must((theme.includes("import('./attendance-institutional-report-v68-10.js?v=6810')")||theme.includes("import('./attendance-institutional-report-v68-11.js?v=6811')"))&&theme.includes("event?.detail?.name!=='groups'"),'report module stays lazy and loads only after Groups is ready');
must(!teacher.includes('attendance-institutional-report-v68-10.js')&&!teacher.includes('attendance-institutional-report-v68-11.js'),'report generator is absent from first paint');
...
must(report.includes('REGISTRO DE ASISTENCIA Y EVALUACIÓN')&&report.includes('Ciclo:')&&report.includes('Sección:')&&report.includes('Asignatura:')&&report.includes('Grupo:')&&report.includes('Profesor:'),'institutional header preserves the official attendance-register structure');
...
must(report.includes('FECHA DE ENTREGA')&&report.includes('doc.line(')&&(report.includes('Vo. Bo. / Coordinación académica')||report.includes('approvalLabel')),'report includes delivery date and signature areas');
...
must(report.includes('Generado con TEDVIO'),'TEDVIO is identified only as document generator/footer');
...
console.log(`\nTEDVIO institutional attendance report regression audit passed using ${current}.`);
