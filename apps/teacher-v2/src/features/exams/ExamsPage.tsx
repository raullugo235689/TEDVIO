import { useAcademicDraft, type AcademicDraft } from '../../core/useAcademicDraft';
...
function ExamEditor({
  workspace,
  initialDraft, initialUpdatedAt = '', onCancel,
}: {
  workspace: ExamWorkspace;
  ...
  const draftKey = ['exam-draft', auth.user?.id, initialDraft.id || `new:${initialDraft.groupId}`];
  const academic = useAcademicDraft(draftKey, { draft: initialDraft, updatedAt: initialUpdatedAt });
  const draft = academic.value.draft;
  function setDraft(update: ExamDraft | ((draft: ExamDraft) => ExamDraft)) { academic.set(current => ({ ...current, draft: typeof update === 'function' ? update(current.draft) : update })); }
  const { online } = useReliability();
  const busy = useIsMutating({ mutationKey: ['exam-write', auth.user?.id] }) > 0;
  const [discard, setDiscard] = useState(false);
  function leave() { if (onCancel) onCancel(); else navigate('/exams'); }
  const [query, setQuery] = useState('');
  ...
  const saveMutation = useMutation({
    mutationKey: ['exam-write', auth.user?.id],
    mutationFn: async ({ markReady, value }: { markReady: boolean; value: typeof academic.value }) => {
      ...
      // Checkpoint the returned ID before changing status so a retry cannot create a duplicate.
      academic.set({ draft: { ...value.draft, id: examId }, updatedAt: saved.exam.updated_at });
      const revision = queryClient.getQueryData<AcademicDraft<typeof academic.value>>(draftKey)?.revision;
      if (markReady) await setExamStatus(auth.user, examId, 'ready');
      return { examId, revision };
    },
    onSuccess: async ({ examId, revision }) => {
      academic.clear(revision);
  ...
  return (
    <div className="view-stack exams-page exam-editor-page">
      <PageHeader
        ...
        actions={<button className="button secondary" disabled={busy} onClick={leave}>← Volver</button>}
      />
      {academic.dirty ? <div className="omr-work-status" role="status"><span>Borrador conservado en esta pestaña. Guarda antes de recargar o cerrar sesión.</span><button className="button ghost" disabled={busy} onClick={() => setDiscard(true)}>Descartar borrador</button></div> : null}
      <fieldset className="omr-review-fields" disabled={busy}>
...
function ExamDetailView({
  ...
  return (
    <div className="view-stack exams-page">
      ...
      <section className="exam-production-panel"><div><span className="eyebrow">MATERIAL DE APLICACIÓN</span><h2>Prepara cada versión para tus alumnos.</h2><p>Cuadernillos, claves separadas y hojas identificadas para corregir con OMR.</p></div><div className="page-actions"><Link className="button primary" to={`/exams/${detail.exam.id}/print`}>Cuadernillo y clave</Link><Link className="button secondary" to={`/omr/${detail.exam.id}/sheets?mode=roster&alternate=1`}>Hojas por alumno</Link><Link className="button ghost" to={`/omr/${detail.exam.id}`}>Revisar con OMR</Link></div></section>
...
export function ExamsPage() {
  ...
  if (workspace.isError && !workspace.data) return <ErrorPanel title="No pude cargar Evaluaciones" detail={workspace.error.message} onRetry={() => workspace.refetch()} />;
  ...
  if (editing) {
    return <ExamEditor key={detail.data.exam.id} workspace={workspace.data} initialDraft={examDraftFromDetail(detail.data)} initialUpdatedAt={detail.data.exam.updated_at} onCancel={() => setEditing(false)} />;
  }
