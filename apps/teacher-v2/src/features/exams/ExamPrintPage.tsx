...
export function ExamPrintPage() {
  ...
  return <div className="exam-print-view" data-print-ready={ready}>
    ...
    <section className="exam-print-controls">
      <label>Documento<select aria-label="Documento" value={teacher ? 'key' : 'student'} onChange={event => change('document', event.target.value)}><option value="student">Cuadernillo del alumno</option><option value="key">Clave docente</option></select></label>
      <label>Versión<select aria-label="Versión" value={version} onChange={event => change('version', event.target.value)}>{exam.versions.map(label => <option key={label} value={label}>Versión {label}</option>)}<option value="all">Todas las versiones</option></select></label>
      <label>Papel<select aria-label="Papel" value={letter ? 'letter' : 'a4'} onChange={event => change('paper', event.target.value)}><option value="a4">A4</option><option value="letter">Carta</option></select></label>
