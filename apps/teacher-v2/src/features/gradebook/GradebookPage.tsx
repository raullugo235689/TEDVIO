...
function DetailView({ detail, periodId, onPeriod }: { detail: GradebookDetail; periodId: string | null; onPeriod: (periodId: string | null) => void }) {
  ...
  return (
    <div className="view-stack gradebook-page">
      <PageHeader eyebrow={groupLabel(detail.group)} title="Calificaciones" detail="Consulta evidencias, promedios y ponderaciones del grupo." actions={<div className="page-actions"><Link className="button ghost" to={`/periods/${detail.group.id}`}>Periodos</Link><button className="button secondary" type="button" onClick={() => exportGradebookCsv(detail, calculation)}>Exportar CSV</button></div>} />
      ...
      <section className="gradebook-next-phase"><Icon name="route" /><div><span className="eyebrow">SEGUIMIENTO ACADÉMICO</span><h2>Consulta la trayectoria de cada alumno.</h2><p>Revisa asistencia, evaluaciones y evolución por periodo en Alumno 360°.</p></div><Link className="button secondary" to={`/groups/${detail.group.id}?tab=students`}>Ver alumnos del grupo</Link></section>
    </div>
  );
}
