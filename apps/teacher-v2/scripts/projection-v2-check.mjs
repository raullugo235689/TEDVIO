...
const app = fs.readFileSync(path.join(root, 'live/projection/app.jsx'), 'utf8');
...
must(app.includes('recoveryStep')&&app.includes('channel !== ownedChannel'),'Projection escala la recuperación y descarta canales obsoletos');
must(app.includes('eventTimer')&&app.includes('document.visibilityState !== "visible"'),'Projection agrupa eventos y pausa consultas ocultas');
must(!app.includes('https://esm.sh')&&!entry.includes('cdnjs.cloudflare.com'),'Projection no depende de runtimes ni QR externos');
