...
async function loadProjection(code) {
  ...
  const s = meta?.[0];
  if (!s) return null;
  const [{ data: counts }, { data: srow }, { data: people }] =
    await Promise.all([
  ...
  if (srow.current_question_id) {
    ...
    if (q?.status === "revealed")
      results =
        (
          await sb.rpc("v2_public_question_results", {
  ...
  if (s.competitive)
    ranking = (await sb.rpc("v2_public_ranking", { p_code: code })).data || [];
  return {
    s: { ...s, ...srow },
    counts: counts?.[0] || { participant_count: 0, answered_count: 0 },
    people: people || [],
    q,
    ranking,
    results,
...
function QR({ code }) {
  ...
  useEffect(() => {
    ...
    new window.QRCode(ref.current, {
      text: `${location.origin}/student-v2/?code=${encodeURIComponent(code)}`,
      width: 230,
      height: 230,
      correctLevel: window.QRCode.CorrectLevel.M,
    });
  }, [code]);
  return h("div", { className: "p2-qr", ref });
}
...
function ConnectionChip({ state, label }) {
  ...
  return h(
    "span",
    { className: `p2-chip p2-connection ${state}` },
    h("span", { className: "p2-dot" }),
    text,
  );
}
function Lobby({ x, code, connection }) {
  return h(
    "div",
    { className: "p2-app" },
    h(
...
function Live({ x, code, tick, connection }) {
  const q = x.q;
...
function App() {
  ...
  useEffect(() => {
    ...
    sync = async () => {
      ...
      syncing = true;
      try {
        const x = await loadProjection(code);
        if (disposed) return;
        if (!x) {
        ...
        if (!disposed) {
          ...
          setError("Reconectando con la sesión…");
        }
      } finally {
        syncing = false;
        if (queued && !disposed) {
    ...
    const poll = setInterval(sync, 2500),
      clock = setInterval(() => setTick((v) => v + 1), 500);
    return () => {
      disposed = true;
      clearInterval(poll);
      clearInterval(clock);
  ...
  if (!state.q) return h(Lobby, { x: state, code, connection });
  return h(Live, { x: state, code, tick, connection });
}
