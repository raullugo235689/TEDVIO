...
function ensureButton(){
  ...
  btn.addEventListener('click', async () => {
    visible = !visible;
    if(visible) await renderRanking(false); else document.querySelector('#tedvioRanking')?.remove();
  });
...
async function sessionSnapshot(code){
  const {data:session,error} = await sb.from('sessions').select('id,current_question_id,status').eq('code',code).maybeSingle();
  if(error || !session) return null;
  let question = null;
  if(session.current_question_id){
    const {data} = await sb.from('questions').select('id,status').eq('id',session.current_question_id).maybeSingle();
    question = data || null;
  }
...
async function renderRanking(auto=false){
  ...
  box.innerHTML = `<button class="tr-close" aria-label="Cerrar">×</button>${auto?'<span class="tr-auto">CLASIFICACIÓN ACTUAL</span>':''}<h3>🏆 Ranking en vivo</h3><div class="tr-sub">Puntos por aciertos + bono por rapidez · Las respuestas abiertas no suman automáticamente.</div>${podium.length?`<div class="tr-podium">${podium.map((r,i)=>`<div><span>${['🥇','🥈','🥉'][i]}</span><b>${esc(r.name)}</b><small>${r.points} pts</small></div>`).join('')}</div>`:''}${ranking.length?ranking.map((r,i)=>`<div class="tr-row"><div class="tr-pos">${i+1}</div><div><div class="tr-name">${esc(r.name)}</div><div class="tr-meta">${r.correct} correctas · ${r.answered} respondidas</div></div><div class="tr-score">${r.points}</div></div>`).join(''):'<div class="tr-empty">Aún no hay alumnos con respuestas registradas.</div>'}`;
  box.querySelector('.tr-close').addEventListener('click',()=>{visible=false;box.remove()});
}

async function autoRankingCheck(){
  const code = getSessionCode();
  if(!code) return;
  const snap = await sessionSnapshot(code);
  if(!snap?.question) return;
  const key = `${snap.question.id}:${snap.question.status}`;
  if(key === lastQuestionState) return;
  lastQuestionState = key;
  if(['closed','revealed'].includes(snap.question.status) && autoShownQuestion !== snap.question.id){
    autoShownQuestion = snap.question.id;
    await renderRanking(true);
  }
  if(snap.question.status === 'live'){
    autoShownQuestion = null;
    if(visible){ visible=false; document.querySelector('#tedvioRanking')?.remove(); }
  }
}
...
setInterval(async()=>{
  ...
  ensureButton();
  await autoRankingCheck();
  const code = getSessionCode();
  if(code !== lastCode && visible){ lastCode = code; await renderRanking(false); }
  else if(visible) await renderRanking(false);
},2500);
