...
async function load(){
  ...
  const{data,error}=await db.from('tedvio_institutions').select('id,name,status,plan,report_display_name,report_logo_path,report_title,report_approver_name,report_approver_title,report_approval_label,report_document_code').in('id',ids).order('name');
...
function render(){
  ...
  o.innerHTML=`<section class="tv6811-shell"><header class="tv6811-head"><div><span>CONFIGURAR TEDVIO</span><h2>Institución y reportes</h2><p>Define la identidad que aparecerá en documentos oficiales sin alterar la vinculación del grupo.</p></div><button class="b-btn secondary" type="button" data-close>×</button></header><div class="tv6811-toolbar"><button class="tv6811-tab active" type="button">Institución</button><button class="tv6811-tab" type="button" data-onboarding>Inicio guiado</button></div><main>${S.institutions.length?S.institutions.map(institutionCard).join(''):'<div class="tv6811-empty"><b>No hay instituciones administrables.</b><span>Esta sección solo aparece para administradores institucionales activos.</span></div>'}</main></section>`;
...
async function saveInstitution(id,button){
  ...
  try{
    ...
    const payload={
      p_institution_id:id,
      p_name:clean(document.querySelector(`#tv6811Name-${id}`)?.value)||null,
